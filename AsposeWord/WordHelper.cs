    ...
    public static class WordHelper
    {
        ...
        public static void SetPlaceHolder(Stream stream, string property, string value)
        {
            using (var document = WordprocessingDocument.Open(stream, true))
            {
                ...
                var texts = document.MainDocumentPart.Document.Body.Descendants<Text>().Where(t => t.Text.Contains(placeHolder)).ToList();

                texts.ForEach(x => x.Text = x.Text.Replace(placeHolder, value));

                document.MainDocumentPart.HeaderParts.ToList().ForEach(x =>
                {
                    var headerTexts = x.Header.Descendants<Text>().Where(y => y.Text.Contains(placeHolder)).ToList();
                    headerTexts.ForEach(y => y.Text = y.Text.Replace(placeHolder, value));
                });
        ...
        public static byte[] ReplacePlaceholdersAndExportToPdf(byte[] template, List<PlaceHolderNameValueDtoBase> nameValueList)
        {
            ...
            using (var templateStream = new MemoryStream())
            {
                ...
                foreach (var nameValue in nameValueList)
                {
                    ...
                    {
                        WordHelper.SetPlaceHolder(templateStream, listDto.Name, listDto.Value);
                    }
                    else if (nameValue is SimplePlaceHolderNameValueDto simpleDto)
                    {
                        WordHelper.SetPlaceHolder(templateStream, simpleDto.Name, simpleDto.Value);
                    }
                }

                return WordHelper.ExportToPdf(templateStream);
            }
        }
